...
for N in 1000; 
do
    DIR=/home/julia/dane/small_test_$N/velvet_${K}_track
...
#     
    echo "Graph"
#     $COMMAND  -g $DIR/LastGraph  -o $DIR/zachlan_v${V}_ --minlen 200 -f 0,$N 0,$N 1,$N 1,$N --profile 2>> $LOG 
    now=$(date '+%d/%m/%Y %H:%M:%S');
...
# for N in 1000000; 
# do
#     DIR=/home/julia/dane/small_test_$N/velvet_${K}_track2
# #     DIR=$INDIR/small_test_$N/velvet_${K}_track
# #     now=$(date '+%d/%m/%Y %H:%M:%S');
# #     echo $N, "$now" >>$LOG
# #     echo "velvet"
# #     time sh small_test2.sh $K $N
# #     
#     echo "Graph"
#     $COMMAND  -g $DIR/LastGraph  -o $DIR/zachlan_v${V}_ --minlen 200 -f 0,$N 0,$N 1,$N 1,$N --profile 2>> $LOG 
#     now=$(date '+%d/%m/%Y %H:%M:%S');
...
# $COMMAND -g "$DIR/LastGraph"  -o "$DIR/zachlan_v${V}_" -f $FILES --paired --minlen 200 --minfc 4 2>> $LOG 
#     now=$(date '+%d/%m/%Y %H:%M:%S');
...
#     now=$(date '+%d/%m/%Y %H:%M:%S');
# echo "all $DIR", "$now" >>$LOG
# $COMMAND -g  "$DIR/meta-velvetg.LastGraph"  -o "$DIR/zachlan_v${V}_" -f $FILES --paired --minlen 200 --minfc 4 2>> $LOG 
#     now=$(date '+%d/%m/%Y %H:%M:%S');
# echo "finished all $DIR", "$now" >>$LOG


sh run_kallisto.sh 21 $DIR zachlan_v${V}_assemblies.fa
